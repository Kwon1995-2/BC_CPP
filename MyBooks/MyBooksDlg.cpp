...
ON_BN_CLICKED(IDC_BTNTEST, &CMyBooksDlg::OnBnClickedBtntest)
END_MESSAGE_MAP()
...
void CMyBooksDlg::OnBnClickedBtnviewA()
{
	...
	myAuthor dlg(1); // --> 중간에 코드를 고쳐서 굳이 1이 들어가지 않아도 괜찮다. --> 나중의 학습을 위해 그대로 남겨놓음
...
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	myPublisher dlg2(1); // --> 중간에 코드를 고쳐서 굳이 1이 들어가지 않아도 괜찮다. --> 나중의 학습을 위해 그대로 남겨놓음
...
//Modalless
void CMyBooksDlg::OnBnClickedBtntest()
{
	if (m_pAuthorDlg == NULL)
	{
		m_pAuthorDlg = new myAuthor(1);
		m_pAuthorDlg->Create(IDD_DIALOG1);
	}
	m_pAuthorDlg->ShowWindow(SW_SHOW);
}
