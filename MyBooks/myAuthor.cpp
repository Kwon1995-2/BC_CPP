...
myAuthor::myAuthor(int mode, CWnd* pParent /*=nullptr*/)
...
BEGIN_MESSAGE_MAP(myAuthor, CDialogEx)
	...
	//ON_BN_CLICKED(IDOK, &myAuthor::OnBnClickedOk)
END_MESSAGE_MAP()
...
void myAuthor::OnBnClickedBtnmodify()
{
	...
	if (ret == IDOK) {
		...
		//sprintf_s(query, "select author_ID, name, phone, e_mail, sns from author where author_ID = (select max(author_ID) from author)");
		sprintf_s(query, "select author_ID, name, phone, e_mail, sns from author where author_ID = %d ", _ttoi(dlg.m_str_AutID));
		mysql_query(&mysql, query);
		MYSQL_RES* result = mysql_store_result(&mysql); //질의 결과
		if (!result)
		{
...
void myAuthor::OnBnClickedBtninsert()
{
	...
	if (ret == IDOK) {
		...
		memset(query, NULL, 200); //배열 초기화
		//sprintf_s(query, "select author_ID, name, phone, e_mail, sns from author where author_ID = (select max(author_ID) from author)");
		sprintf_s(query, "select author_ID, name, phone, e_mail, sns from author where author_ID = %d ",_ttoi(dlg.m_strIN_AutID));
		mysql_query(&mysql, query);
		MYSQL_RES* result = mysql_store_result(&mysql); //질의 결과
		if (!result)
		{
		...
		/*m_strIN_AutID = dlg.m_strIN_AutID;
		CDialogEx::OnOK();*/
	}
}
