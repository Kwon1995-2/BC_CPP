...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Text(pDX, IDC_SVR_IP, m_strSvrIP);
	DDX_Text(pDX, IDC_SVR_PORT, m_nPort);
	DDX_Text(pDX, IDC_STRING1, m_strMsg);
...
BEGIN_MESSAGE_MAP(Cstd1WinDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &Cstd1WinDlg::OnBnClickedButton1)
END_MESSAGE_MAP()
...
void Cstd1WinDlg::OnBnClickedButton1()
{
	...
	{
		m_strResult.Format("socket() failed![%d]\n", WSAGetLastError());
		UpdateData(FALSE);
		return;
	}
	...
	inet_pton(AF_INET, m_nPort, &name.sin_addr);
	...
	// send
	if (send(sock, m_strMsg, m_strMsg.GetLength(), 0) == SOCKET_ERROR)
	{
		m_strResult.Format("send() failed![% d]\n", WSAGetLastError());
		return;
	}
	else
